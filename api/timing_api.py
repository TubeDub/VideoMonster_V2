"""Timing Agent v1.0 API — timing report endpoint."""
...
logger = logging.getLogger("tubedub.timing_api")
...
def _report_path_for_task(task_id: str) -> Path | None:
    ...
    report_path = info.get("timing_report_path")
    if report_path:
        p = Path(report_path)
        if p.is_file():
            return p
    ...
    if project_uuid:
        candidate = _MANIFESTS_DIR / project_uuid / "timing_report.json"
        if candidate.is_file():
            return candidate
    return None


@bp.get("/api/timing/<task_id>")
def api_timing_report(task_id: str):
    ...
    if not report_path:
        return jsonify({"ok": False, "error": "timing_report not found", "task_id": safe}), 404

    report = _load_json(report_path)
    if not report:
        return jsonify({"ok": False, "error": "timing_report unreadable", "task_id": safe}), 404
