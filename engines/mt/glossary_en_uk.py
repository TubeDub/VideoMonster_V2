...
# Marian mangling repairs (UK side) — Stage 16 George Jr. quality.
...
def apply_uk_marian_repairs(text: str) -> str:
    ...
    # Collapse duplicated «Зоряні війни» / Lucas closers.
    out = re.sub(
        r"(Джордж\s+Лукас[^.?!]*Зоряні\s+війни[^.?!]*[.?!]\s*){2,}",
        r"\1",
        out,
        flags=re.IGNORECASE,
    )
    out = re.sub(
        r"(а\s+його\s+кінофраншиза\s+[—\-]\s+це\s+«Зоряні\s+війни»\.\s*){2,}",
        r"\1",
        out,
        flags=re.IGNORECASE,
    )
